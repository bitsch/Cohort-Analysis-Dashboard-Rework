...
def transform_lifecycle_csv_to_interval_csv(df):
    ...
    df = df.sort_values(xes.DEFAULT_TIMESTAMP_KEY).groupby(by = [xes.DEFAULT_NAME_KEY, "case:concept:name",  xes.DEFAULT_TRANSITION_KEY]).agg({xes.DEFAULT_TIMESTAMP_KEY: list})
    ...
    df = df.rename({"start" : xes.DEFAULT_START_TIMESTAMP_KEY, 
                    "complete" : xes.DEFAULT_TIMESTAMP_KEY}, axis = 1)
