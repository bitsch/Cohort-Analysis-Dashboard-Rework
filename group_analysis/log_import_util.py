...
def csv_create_date_range_frame(log, Groups, parameters = None, freq = 'H', interval = False):
    ...
    if interval:
        ...
        date_range = pd.date_range(dt_utils.datetime_floor(min_time, "D"), dt_utils.datetime_ceil(max_time, "D"), freq = settings.INTERVAL_GRANULARITY)
...
def xes_create_date_range_frame(log, Groups, min_time, max_time, parameters = None, freq = 'H', interval = False):
    ...
    date_range = pd.date_range(dt_utils.datetime_floor(min_time, "D"), dt_utils.datetime_ceil(max_time, "D"), freq = freq)
    date_frame = pd.DataFrame({group.name:0 for group in Groups}, index = date_range)
            ...
            for group in Groups: 
                ...
                if event["concept:name"] in group.members:
                    ...
                    if interval: 
                        ...
                        #TODO Replace default Name
                        date_frame.loc[dt_utils.datetime_floor(event["time:timestamp"], freq = freq), group.name] += 1
