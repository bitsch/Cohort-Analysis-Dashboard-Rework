...
def create_analysis_dataframe(log, net, replayed_traces, group,log_information):
    ...
    global timeframe
    print(group.name)
    print(event_logs_path+'_'+log_information['log_name']+'_'+group.name+'_'+timeframe+".csv")
    try:
        df=pd.read_csv(event_logs_path+'_'+log_information['log_name']+'_'+group.name+'_'+timeframe+".csv")
        df['TimeSpent']=pd.to_timedelta(df['TimeSpent'])
        df['date']=pd.to_datetime(df['date'])
    except:
        df=createZone(log,net, replayed_traces, group)
        df.to_csv(event_logs_path+'_'+log_information['log_name']+'_'+group.name+'_'+timeframe+".csv", index = False, header=True)
