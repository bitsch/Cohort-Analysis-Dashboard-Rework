...
def demo_hospital(log, log_format, log_information):
    ...
    Groups = [
        Group(
            name="Inform Authority",
            members=["Inform Authority Fill Form", "Inform Authority Send Form"],
        ),
        Group(
            name="Check Treat",
            members=["Check Treatment A1", "Check Treatment A2", "Check Treatment A3"],
        ),
    ]
    ...
    concurrency_plt_div = plotting.concurrency_plot_factory(
        date_frame, Groups, freq="D", aggregate=max
    )
    timeframe_plt_div = plotting.amplitude_plot_factory(date_frame, Groups)

    context = {
        "concurrency_plt_div": concurrency_plt_div,
