...
def generate_transformed_log_XES(
    original_file, abstracted_patterns, abstracted_timestamps, transformed_file
):
    """
    ...
    try:
        ...
        transformed_log_content = transformed_log_content + """\t<trace>\n"""
        ...
        for index, activity in enumerate(abstracted_patterns):
            if (
                "|" not in abstracted_patterns[index]
                or abstracted_patterns[index] != "|"
            ):
                # To check if there are two timestamps: start and end time
                if type(abstracted_timestamps[index]) == list:

                    transformed_log_content = transformed_log_content + str(
                        """\t\t<event>\n"""
                ...
                transformed_log_content = (
                    ...
                    """\t\t<string key="concept:name" """
                    """value="{case_id}"/>\n""".format(case_id=case_id)
                )  # to get value next to |
                case_id = case_id + 1

        # for last trace only
        transformed_log_content = transformed_log_content + """\t</trace>\n"""

        transformed_log_content = transformed_log_content + """</log>\n"""
...
def generate_transformed_log_CSV(
    ...
    try:
        ...
        transformed_log_content = [
            ["case:concept:name", "Start Timestamp", "Complete Timestamp", "Activity"]
        ]
                ...
                if (
                    ...
                    if type(abstracted_timestamps[index]) == list:
                        writer.writerow(
                            [
                                case_number,
                                abstracted_timestamps[index][0],
                                abstracted_timestamps[index][1],
                                activity,
                            ]
                        )
                        transformed_log_content = transformed_log_content + [
                            [
                                str(case_number),
                                str(abstracted_timestamps[index][0]),
                                str(abstracted_timestamps[index][1]),
                                activity,
                            ]
                        ]
                    else:
                        ...
                        transformed_log_content = transformed_log_content + [
                            [
                                str(case_number),
                                str(abstracted_timestamps[index]),
                                str(abstracted_timestamps[index]),
                                activity,
                            ]
                        ]
...
def perform_transformation(event_log, export_log, export_model, patterns):
    """
    ...
    try:
        ...
        filename = (
            event_log.replace(".xes", "_patterns." + ext)
        ...
        if (event_log.endswith(".xes") or event_log.endswith(".XES")) or (
            ...
            if "_transformed" not in event_log:
                ...
                transformed_filename = filepath + "\\" + name + "_transformed.XES"
                display_name = name + "_transformed.XES"

                exported_transformed_filename = (
                    filepath + "\\" + name + "_transformed_exported.XES"
                )
                display_name_exported = name + "_transformed_exported.XES"

                transformed_model_name = filepath + "\\" + name + "_transformed.svg"
                display_name_model = name + "_transformed.svg"

                exported_transformed_model_name = (
                    filepath + "\\" + name + "_transformed_exported.svg"
                )
                display_name_model_exported = name + "_transformed_exported.svg"

            else:
                transformed_filename = filepath + "\\" + name + ".XES"
                display_name = name + "_transformed.XES"

                exported_transformed_filename = filepath + "\\" + name + "_exported.XES"
                display_name_exported = name + "_exported.XES"

                transformed_model_name = filepath + "\\" + name + ".svg"
                display_name_model = name + ".svg"

                exported_transformed_model_name = (
                    filepath + "\\" + name + "_exported.svg"
                )
        ...
        # concatenated_traces, concatenated_timestamps = read_log(log)
        abstracted_traces, abstracted_timestamps = perform_abstractions(
            patterns, pattern_dic, concatenated_traces, concatenated_timestamps
        )
        ...
        display_message("""Log is transformed""".format(display_name))
        ...
        if export_log:
            ...
            display_message("""Log exported as {}""".format(display_name_exported))
        if export_model:

            utils.export_process_model(dfg, log, exported_transformed_model_name)

            display_message(
                "Process Model exported as {}".format(display_name_model_exported)
            )
        return True
    except Exception as e:
        print("Exception!!!... -> " + str(e))
...
if __name__ == "__main__":
    ...
    try:
        ...
        parser.add_argument(
            ...
            .path.name.replace(".xes", "_patterns.json")
        ...
        perform_transformation(
            args.path.name, args.downloadLog, args.downloadModel, args.pattern
        )
