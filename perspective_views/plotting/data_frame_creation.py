...
def create_df_variant(log, file_format, log_information):
    ...
    if file_format == "csv":
        ...
        if log_information["log_type"] == "noninterval": 
            ...
            df = log.groupby("case:concept:name").agg({xes.DEFAULT_NAME_KEY : lambda x : tuple(x)})
            ...
            df_times = log.groupby(by = ["case:concept:name"]).agg({xes.DEFAULT_TIMESTAMP_KEY : [min, max]})
            df_times = df_times.droplevel(0, axis = 1)

            # Compute Variant based on the Complete Event
            df_variants = log.loc[log[xes.DEFAULT_TRANSITION_KEY] == "complete"].groupby(by = ["case:concept:name"]).agg({xes.DEFAULT_NAME_KEY : lambda x : tuple(x)})

            # Full index join, between the min and max times of each case and the variant
            df = df_times.join(df_variants)
            df = df.rename({"min" : "Start", "max" : "End", xes.DEFAULT_NAME_KEY : "variant"}, axis = 1)
            df["Name"] = [str(x) for x in df.index]
